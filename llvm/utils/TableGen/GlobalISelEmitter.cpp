...
Expected<action_iterator> GlobalISelEmitter::importExplicitUseRenderer(
  ...
  if (!DstChild->isLeaf()) {
    ...
    if (DstChild->getOperator()->isSubClassOf("Instruction")) {
      CodeGenInstruction &DstI = Target.getInstruction(DstChild->getOperator());
      if (DstI.Operands.NumDefs != 1)
...
Error GlobalISelEmitter::importImplicitDefRenderers(
  ...
  for (int I = 0; I < DstINumImplicitDefs; ++I) {
    ...
    M.addAction<ConstrainOperandToRegClassAction>(
        ...
        *Target.getRegBank().getRegClassForRegister(PhysOutput));
  }
  return Error::success();
}
...
Expected<RuleMatcher> GlobalISelEmitter::runOnPattern(const PatternToMatch &P) {
  ...
  for (unsigned OpIdx = 0; OpIdx != SrcNumResults; ++OpIdx) {
    ...
    if (OpIdx < DstI.Operands.NumDefs) {
      ...
      RC = Target.getRegBank().getRegClassForRegister(DstIOpRec);
      if (!RC)
