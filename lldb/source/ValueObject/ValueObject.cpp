  ...
  if (IsScalarType()) {
    ...
    if (!synthetic_child_sp) {
      ...
      ValueObjectChild *synthetic_child = new ValueObjectChild(
          *this, GetCompilerType(), index_const_str, GetBitSize().value_or(0),
...
ValueObjectSP ValueObject::Dereference(Status &error) {
  ...
  if (is_pointer_or_reference_type) {
    ...
    auto child_compiler_type_or_err = compiler_type.GetChildCompilerTypeAtIndex(
        &exe_ctx, 0, transparent_pointers, omit_empty_base_classes,
        ignore_array_bounds, child_name_str, child_bit_size, child_bit_offset,
    ...
    if (child_compiler_type && child_bit_size) {
      ConstString child_name;
      if (!child_name_str.empty())
        child_name.SetCString(child_name_str.c_str());

      m_deref_valobj = new ValueObjectChild(
          *this, child_compiler_type, child_name, child_bit_size,
          child_bit_offset, child_bitfield_bit_size, child_bitfield_bit_offset,
    ...
    if (!m_deref_valobj) {
      ...
      if (Language::LanguageIsObjC(GetPreferredDisplayLanguage()) &&
        ...
        if (child_compiler_type) {
          ...
          m_deref_valobj = new ValueObjectChild(
              *this, child_compiler_type, child_name, child_bit_size,
              child_bit_offset, child_bitfield_bit_size,
              child_bitfield_bit_offset, child_is_base_class,
              child_is_deref_of_parent, eAddressTypeInvalid, language_flags);
        }
      }
