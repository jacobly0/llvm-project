      ...
      if (format_cstring) {
        ...
        if (ValueObject *ptr_valobj =
          ...
          if (Target *target = exe_ctx.GetTargetPtr()) {
            ...
            if (max_len > length)
              max_len = length;
            if (ValueObject::AddrAndType ptr_val =
                    ptr_valobj->GetPointerValue();
                ptr_val.address != LLDB_INVALID_ADDRESS) {
              Status error;
              ...
              case eAddressTypeHost: {
                const uint8_t *host_ptr =
                    reinterpret_cast<const uint8_t *>(ptr_val.address);
...
std::string TypeFormatImpl_Format::GetDescription() const {
  StreamString sstr;
...
std::string TypeFormatImpl_EnumType::GetDescription() const {
  StreamString sstr;
