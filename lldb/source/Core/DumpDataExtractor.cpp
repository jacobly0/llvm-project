...
static bool DumpFloat(const DataExtractor &DE, Stream *s, offset_t &offset,
  ...
  if (exe_scope) {
    ...
    if (!flt_semantics) {
      ...
      flt_semantics = &GetFloatSemantics(target_sp, lang.AsLanguageType(),
                                         item_byte_size, format);
    }
  }
...
lldb::offset_t lldb_private::DumpDataExtractor(
  ...
  for (uint32_t count = 0; DE.ValidOffset(offset) && count < item_count;
    ...
    case eFormatComplex:
      if (!DumpFloat(DE, s, offset, item_byte_size / 2, exe_scope, item_format,
                     flt_semantics))
        return offset;
      s->PutCString(" + ");
      if (!DumpFloat(DE, s, offset, item_byte_size / 2, exe_scope, item_format,
    ...
    case eFormatFloat:
      if (!DumpFloat(DE, s, offset, item_byte_size, exe_scope, item_format,
                     flt_semantics))
        return offset;
      break;
