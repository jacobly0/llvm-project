...
public:
  ...
  lldb::ChildCacheState Update() override {
    ...
    if (ValueObjectZig *elem_type = ValueObjectZig::dyn_cast_if_present(
      ...
      if (ZigType *zig_elem_type =
              llvm::dyn_cast_if_present<ZigStructType>(zig_elem_value))
  ...
  size_t GetIndexOfChildWithName(ConstString name) override {
    if (uint32_t field_idx =
            m_elem_type.GetIndexOfFieldWithName(name.GetCString());
        field_idx != UINT32_MAX)
      return m_elem_type.GetNumFields() + field_idx;
    if (m_len) {
      llvm::StringRef name_ref = name.GetStringRef();
      uint64_t len = m_len->GetValueAsUnsigned(0);
      uint32_t idx;
      if (name_ref.consume_front("[") && name_ref.consume_back("]") &&
          name_ref.getAsInteger(10, idx) && idx < len)
        return idx;
    }
    return UINT32_MAX;
  }
