...
static void LoadStdFormatters(lldb::TypeCategoryImplSP zig_category_sp) {
  ...
  formatters::AddStringSummary(
      zig_category_sp, "len=${svar%#} capacity=${svar.header.capacity}",
      "^std\\.hash_map\\.Custom\\(.+\\)$", TypeSummaryImpl::Flags(), true);
  formatters::AddCXXSynthetic(
      zig_category_sp, formatters::ZigStdHashMapSyntheticFrontEndCreator,
      "zig std.hash_map synthetic children",
      "^std\\.hash_map\\.Custom\\(.+\\)$", SyntheticChildren::Flags(), true);
  ...
  llvm::call_once(g_initialize, []() {
    ...
    g_formatters.push_back(
        [](ValueObject &valobj, lldb::DynamicValueType,
           FormatManager &) -> CXXFunctionSummaryFormat::SharedPointer {
          static CXXFunctionSummaryFormat::SharedPointer formatter_sp(
              new CXXFunctionSummaryFormat(
                  TypeSummaryImpl::Flags().SetDontShowValue(),
    ...
    g_formatters.push_back(
        [](ValueObject &valobj, lldb::DynamicValueType,
           FormatManager &) -> StringSummaryFormat::SharedPointer {
          ...
          if (auto type_system =
            ...
            if (ZigPointerType *zig_type =
                    llvm::dyn_cast_if_present<ZigPointerType>(
