...
LLDB_PLUGIN_DEFINE_ADV(ZigHighlighter, HighlighterZig)
...
void ZigHighlighter::Highlight(const HighlightStyle &options,
                               ...
                               Stream &result) const {
  if (cursor_pos && *cursor_pos >= line.size()) cursor_pos = std::nullopt;
  ...
  while (true) {
    ...
    case ZigLexer::Token::Kind::NonZigIdentifier:
...
Highlighter *ZigHighlighter::CreateInstance(lldb::LanguageType language) {
  return language == lldb::eLanguageTypeZig ? new ZigHighlighter : nullptr;
}

void ZigHighlighter::Initialize() {
  PluginManager::RegisterPlugin(GetPluginNameStatic(), GetPluginNameStatic(),
                                CreateInstance);
}

void ZigHighlighter::Terminate() {
