...
#include "Plugins/TypeSystem/Zig/VariableZig.h"
...
TypeSP DWARFASTParserZig::ParseTypeFromDWARF(const SymbolContext &sc,
  ...
  if (DWARFDIE signature_die = attrs.signature.Reference()) {
    ...
    case DW_TAG_unspecified_type:
      ...
      else if (attrs.name == "@EnumLiteral()")
...
VariableSP
DWARFASTParserZig::ParseVariableFromDWARF(const SymbolContext &sc,
                                          const DWARFDIE &die,
  ...
  for (size_t i = 0; i < attributes.Size(); ++i) {
    ...
    case DW_AT_ZIG_comptime_value:
      comptime_value_die = form_value.Reference();
      break;
  ...
  DWARFExpressionList location_list = [&] {
    if (location_form.IsValid())
      return SymbolFileDWARF::GetExprListFromAtLocation(
          location_form, module_sp, die, func_low_pc);
    if (const_value_form.IsValid())
      return SymbolFileDWARF::GetExprListFromAtConstValue(const_value_form,
                                                          module_sp, die);
    return DWARFExpressionList(module_sp, DWARFExpression(), die.GetCU());
  }();

  const DWARFDIE parent_context_die =
      dwarf->GetDeclContextDIEContainingDIE(die);
  const DWARFDIE sc_parent_die = dwarf->GetParentSymbolContextDIE(die);
  ...
  if (tag == DW_TAG_formal_parameter)
    ...
    bool is_static_lifetime =
        location_form.IsValid() && !location_list.IsValid();
    ...
    if (is_static_lifetime) {
      ...
      if (debug_map_symfile) {
        ...
        const lldb::addr_t exe_file_addr =
            debug_map_symfile->LinkOSOFileAddress(dwarf, location_DW_OP_addr);
  ...
  auto type_sp = std::make_shared<SymbolFileType>(
      *dwarf, type_die_form.Reference().GetID());
  return std::make_shared<VariableZig>(
      die.GetID(), name, mangled, type_sp, scope, symbol_context_scope,
      scope_ranges, &decl, location_list,
      ParseComptimeValue(comptime_value_die), is_external, is_artificial,
