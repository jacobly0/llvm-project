...
TypeSP DWARFASTParserZig::ParseTypeFromDWARF(const SymbolContext &sc,
  ...
  if (DWARFDIE signature_die = attrs.signature.Reference()) {
    ...
    case DW_TAG_unspecified_type:
      ...
      else if (attrs.name == "@TypeOf(.enum_literal)")
...
Function *
DWARFASTParserZig::ParseFunctionFromDWARF(CompileUnit &comp_unit,
                                          const DWARFDIE &die,
  ...
  case ZigValue::Kind::PackedUnionType:
    for (DWARFDIE child_die : die.children()) {
      if (child_die.Tag() != DW_TAG_member)
        continue;
      const MemberAttributes attrs(child_die);
      Type *member_type = attrs.type.Reference().ResolveType();
      if (!member_type) {
        if (attrs.name)
          module_sp->ReportError(
              "{0:x8}: DW_TAG_member '{1}' refers to type {2:x16}"
              " which was unable to be parsed",
              uint32_t(die.GetID()), attrs.name,
              attrs.type.Reference().GetOffset());
        else
      ...
      ZigValue *member_zig_default_value =
          ParseValue(attrs.default_value, member_zig_type);
      if ((member_zig_default_value != nullptr) !=
          attrs.default_value.IsValid())
        break;
      fields.emplace_back(attrs.is_comptime, attrs.name, member_zig_type,
                          attrs.align, member_zig_default_value,
                          attrs.bit_offset);
    }
    break;
  case ZigValue::Kind::TaggedUnionType:
    ...
    for (DWARFDIE variant_die : part_die.children()) {
      if (variant_die.Tag() != DW_TAG_variant)
        continue;
      for (DWARFDIE child_die : variant_die.children()) {
        if (child_die.Tag() != DW_TAG_member)
          continue;
        const MemberAttributes attrs(child_die);
        Type *member_type = attrs.type.Reference().ResolveType();
        if (!member_type) {
          if (attrs.name)
            ...
            module_sp->ReportError(
                ...
                " which was unable to be parsed",
                uint32_t(die.GetID()), attrs.type.Reference().GetOffset());
          return false;
        }
        CompilerType member_zig_type = member_type->GetLayoutCompilerType();
        member_zig_type.GetCompleteType();
        if (attrs.default_value.IsValid())
          return false;
        fields.emplace_back(attrs.is_comptime, attrs.name,
                            m_ast.UnwrapType(member_zig_type), attrs.align,
                            nullptr, attrs.bit_offset);
        break;
      }
    }
    break;
...
CompilerDecl DWARFASTParserZig::GetDeclForUIDFromDWARF(const DWARFDIE &die) {
  ...
  case DW_TAG_variable:
    for (llvm::dwarf::Attribute ref_attr :
         {DW_AT_specification, DW_AT_abstract_origin})
      if (DWARFDIE ref_die = die.GetReferencedDIE(ref_attr))
        if (CompilerDecl decl = SymbolFileDWARF::GetDecl(ref_die)) {
          zig_decl = m_ast.UnwrapDecl(decl);
          break;
        }
    if (zig_decl)
      break;
    ...
    case DW_TAG_formal_parameter: {
      ...
      if (const_form_value.IsValid()) {
        ZigValue *zig_const_value = ParseValue(const_form_value, zig_type);
        if (zig_const_value)
    ...
    default:
      llvm_unreachable("already validated tag");
    }
    break;
  case DW_TAG_enumeration_type:
  case DW_TAG_structure_type:
