...
ZigData::ZigData(TypeSystemZig *type_system, ZigType *type,
  ...
  std::uninitialized_copy(data.begin(), data.end(), getTrailingObjects());
}
...
void ZigEnumLiteralType::PrintName(Stream &s) const { s << "@EnumLiteral()"; }
...
llvm::ArrayRef<ZigTagField> ZigTagTypeImpl<DerivedType>::GetFields() const {
  return llvm::ArrayRef(TrailingObjects::getTrailingObjects(), GetNumFields());
}
...
ZigTagTypeImpl<DerivedType>::ZigTagTypeImpl(TypeSystemZig *type_system,
  ...
  std::uninitialized_copy(fields.begin(), fields.end(),
                          TrailingObjects::getTrailingObjects());
}
...
ZigTupleType::ZigTupleType(TypeSystemZig *type_system,
  ...
  std::uninitialized_copy(fields.begin(), fields.end(), getTrailingObjects());
}
