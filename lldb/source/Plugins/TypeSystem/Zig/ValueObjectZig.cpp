...
bool ValueObjectZig::GetValueAsCString(const TypeFormatImpl &format,
                                       std::string &destination) {
  auto [type_system, zig_value] = UnwrapZigValue();
  StreamString sstr;
  switch (format.GetType()) {
  default:
    type_system->DumpValue(zig_value, sstr);
    break;
  case TypeFormatImpl::Type::eTypeFormat:
    type_system->DumpValue(
        zig_value, sstr,
        static_cast<const TypeFormatImpl_Format &>(format).GetFormat());
    break;
  }
  destination = sstr.GetString();
...
ValueObjectSP ValueObjectZig::Clone(llvm::StringRef new_name) {
...
ValueObjectSP ValueObjectZig::Substitute() {
    ...
    if (StackFrameSP frame = GetFrameSP()) {
      ...
      for (VariableSP var : *vars)
        if (var->GetDecl() == var_decl) {
          m_substitute = var->CreateValueObject(frame.get());
  ...
  case ZigValue::Kind::Data: {
    ...
    m_substitute = ValueObject::CreateValueObjectFromData(
        ...
        GetExecutionContextRef(), type_system->WrapType(zig_value->GetType()));
    break;
  }
  case ZigValue::Kind::BoolFalse:
  case ZigValue::Kind::BoolTrue:
    m_substitute = ValueObject::CreateValueObjectFromBool(
        GetExecutionContextRef(), type_system,
        llvm::cast<ZigBool>(zig_value)->GetValue(), GetName());
    break;
  case ZigValue::Kind::Int:
    m_substitute = ValueObject::CreateValueObjectFromAPInt(
        GetExecutionContextRef(), llvm::cast<ZigInt>(zig_value)->GetValue(),
  ...
  case ZigValue::Kind::Float:
    m_substitute = ValueObject::CreateValueObjectFromAPFloat(
        GetExecutionContextRef(), llvm::cast<ZigFloat>(zig_value)->GetValue(),
        type_system->WrapType(zig_value->GetType()), GetName());
    break;
  case ZigValue::Kind::Tag:
    m_substitute = ValueObject::CreateValueObjectFromAPInt(
        GetExecutionContextRef(),
        llvm::cast<ZigTag>(zig_value)->GetValue()->GetValue(),
        type_system->WrapType(zig_value->GetType()), GetName());
    break;
  }
  return m_substitute;
