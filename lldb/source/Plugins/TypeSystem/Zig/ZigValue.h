...
public:
  ...
  private:
    ...
    Marker(const ZigValue &value) : m_value(value), m_was_marked(value.m_mark) {
      value.m_mark = true;
    }
...
public:
  ...
  virtual bool AnyComptimeOnly() const = 0;
...
public:
  ...
  bool AnyComptimeOnly() const final { return true; }
...
public:
  ...
  bool AnyComptimeOnly() const final { return false; }

protected:
  void PrintName(Stream &s) const final;
};

class ZigVoid final : public ZigValue {
public:
  static bool classof(const ZigValue *value) {
    return value->GetKind() == Kind::Void;
  }

  ZigVoid(TypeSystemZig *type_system);

  ZigVoidType *GetType() const {
    return llvm::cast<ZigVoidType>(ZigValue::GetType());
  }
};
...
class ZigUndefined final : public ZigValue {
public:
  static bool classof(const ZigValue *value) {
    return value->GetKind() == Kind::Undefined;
  }

  ZigUndefined(TypeSystemZig *type_system);

  ZigUndefinedType *GetType() const {
    return llvm::cast<ZigUndefinedType>(ZigValue::GetType());
  }
};
...
class ZigNull final : public ZigValue {
public:
  static bool classof(const ZigValue *value) {
    return value->GetKind() == Kind::Null;
  }

  ZigNull(TypeSystemZig *type_system);

  ZigNullType *GetType() const {
    return llvm::cast<ZigNullType>(ZigValue::GetType());
  }
};
...
public:
  ...
  bool AnyComptimeOnly() const final {
    return Mark() && GetChildType()->AnyComptimeOnly();
  }
...
public:
  ...
  bool AnyComptimeOnly() const final {
    return GetBackingType()->AnyComptimeOnly();
  }
...
public:
  ...
  bool AnyComptimeOnly() const final {
    return Mark() && (GetErrorSet()->AnyComptimeOnly() ||
                      GetPayload()->AnyComptimeOnly());
  }
...
public:
  ...
  bool AnyComptimeOnly() const final {
    return GetLength() && Mark() && GetChildType()->AnyComptimeOnly();
  }
  ...
  bool AnyComptimeOnly() const final {
    if (Marker marker = Mark())
      for (const ZigRecordField &field : GetFields())
        if (field.IsComptime() || field.GetType()->AnyComptimeOnly())
          return true;
    return false;
  }
