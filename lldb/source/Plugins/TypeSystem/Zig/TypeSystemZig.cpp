...
struct TypeSystemZig::ZigFunctionTypeKeyInfo {
  ...
  public:
    ...
    friend llvm::hash_code hash_value(const Key &key) {
      return llvm::hash_combine(key.m_param_types, key.m_is_var_args, key.m_ret_type);
    }
...
ZigValue *TypeSystemZig::GetData(ZigType *type, llvm::ArrayRef<uint8_t> data) {
  if (type->GetByteSize() != data.size())
    return nullptr;
  if (llvm::isa<ZigVoidType>(type)) {
    assert(data.size() == 0 && "wrong data size");
    return GetVoid();
  }
  if (llvm::isa<ZigUndefinedType>(type)) {
    assert(data.size() == 0 && "wrong data size");
    return GetUndefined();
  }
  if (llvm::isa<ZigNullType>(type)) {
    assert(data.size() == 0 && "wrong data size");
    return GetNull();
  }
...
ZigVoid *TypeSystemZig::GetVoid() {
  if (!m_void) {
    m_void = m_allocator.Allocate<ZigVoid>();
    new (m_void) ZigVoid(this);
  }
  return m_void;
}
...
ZigUndefined *TypeSystemZig::GetUndefined() {
  if (!m_undefined) {
    m_undefined = m_allocator.Allocate<ZigUndefined>();
    new (m_undefined) ZigUndefined(this);
  }
  return m_undefined;
}
ZigNull *TypeSystemZig::GetNull() {
  if (!m_null) {
    m_null = m_allocator.Allocate<ZigNull>();
    new (m_null) ZigNull(this);
  }
  return m_null;
}
...
TypeSystemZig::DeclGetCompilerContext(void *opaque_decl) {
  ...
  case ZigValue::Kind::Void:
  case ZigValue::Kind::ComptimeInt:
  case ZigValue::Kind::ComptimeFloat:
  case ZigValue::Kind::Undefined:
  case ZigValue::Kind::Null:
...
TypeSystemZig::DeclGetConstantValueObject(void *opaque_decl,
  ...
  case ZigValue::Kind::Function:
    llvm_unreachable("already unwrapped");
  case ZigValue::Kind::Data:
    if (!zig_const->GetType()->AnyComptimeOnly()) {
  ...
  case ZigValue::Kind::ComptimeInt:
    return ValueObject::CreateValueObjectFromAPInt(
        ...
        llvm::cast<ZigComptimeInt>(zig_const->GetValue())->GetValue(),
        WrapType(zig_const->GetType()), zig_const->GetName());
  case ZigValue::Kind::ComptimeFloat:
    return ValueObject::CreateValueObjectFromAPFloat(
        exe_scope->CalculateTarget(),
        llvm::cast<ZigComptimeFloat>(zig_const->GetValue())->GetValue(),
        WrapType(zig_const->GetType()), zig_const->GetName());
  case ZigValue::Kind::Undefined:
  case ZigValue::Kind::Null:
    break;
...
bool TypeSystemZig::IsDefined(opaque_compiler_type_t type) {
  if (ZigRecordType *zig_type = llvm::dyn_cast<ZigRecordType>(UnwrapType(type)))
    return zig_type->GetFields().data();
  return type;
}
...
TypeSystemZig::GetNumChildren(opaque_compiler_type_t type,
  ...
  case ZigValue::Kind::PointerType: {
    ...
    switch (zig_pointer_type->GetSize()) {
    case ZigPointerType::Size::One: {
      ZigType *zig_child_type = zig_pointer_type->GetChildType();
      if (IsAggregateType(zig_child_type->AsOpaqueType())) {
        if (auto num_children_or_err =
                GetNumChildren(zig_child_type->AsOpaqueType(),
  ...
  case ZigValue::Kind::TaggedUnionType:
  case ZigValue::Kind::PackedUnionType:
    num_children = (zig_type->GetKind() == ZigValue::Kind::TaggedUnionType) +
                   llvm::cast<ZigRecordType>(zig_type)->GetFields().size();
    break;
  }
...
uint32_t TypeSystemZig::GetNumFields(opaque_compiler_type_t type) {
  ZigType *zig_type = UnwrapType(type);
  if (ZigRecordType *zig_record_type =
  ...
  else if (llvm::isa_and_present<ZigErrorUnionType>(zig_type))
    return 2;
  return 0;
}
...
llvm::Expected<CompilerType> TypeSystemZig::GetChildCompilerTypeAtIndex(
  ...
  case ZigValue::Kind::OptionalType:
    if (idx_is_valid) {
      ...
      case 0:
        ...
        child_bit_offset = child_type->GetBitSize();
        child_bit_size = 1;
        return WrapType(m_bool_type);
      case 1:
        if (valobj)
        ...
        child_name += ".?";
        child_bit_size = child_type->GetByteSize() * UINT64_C(8);
      ...
      case 0:
        ...
        if (error_union_type->GetErrorSet()->GetAlign() <=
          ...
          child_bit_offset =
              ...
              UINT64_C(8);
        child_bit_size =
            error_union_type->GetErrorSet()->GetByteSize() * UINT64_C(8);
      ...
      case 1:
        ...
        if (error_union_type->GetErrorSet()->GetAlign() >
          ...
          child_bit_offset =
              ...
              UINT64_C(8);
        child_bit_size =
            error_union_type->GetPayload()->GetByteSize() * UINT64_C(8);
  ...
  case ZigValue::Kind::TaggedUnionType:
    if (idx == 0) {
      ...
      child_bit_size =
          tagged_union_type->GetTagType()->GetByteSize() * UINT64_C(8);
  ...
  case ZigValue::Kind::PackedUnionType:
    if (idx_is_valid) {
      ...
      CompilerType field_type = WrapType(field.GetType());
      ...
      case ZigValue::Kind::UnionType:
      case ZigValue::Kind::TaggedUnionType:
        child_bit_size = field.GetType()->GetByteSize() * UINT64_C(8);
        break;
      case ZigValue::Kind::PackedStructType:
      case ZigValue::Kind::PackedUnionType:
        child_bit_size = field.GetType()->GetBitSize();
        break;
      }
...
bool TypeSystemZig::DumpValue(ZigValue *value, Stream &s,
  ...
  case ZigValue::Kind::Alias: {
    ...
    return DumpValue(constant->GetValue(), s, exe_ctx);
  }
  case ZigValue::Kind::Variable: {
  ...
  case ZigValue::Kind::Slice:
    s << '&';
    return DumpValue(llvm::cast<ZigPointer>(value)->GetPointee(), s, exe_ctx);
  case ZigValue::Kind::Tag: {
...
bool TypeSystemZig::DumpTypeDecl(opaque_compiler_type_t type, Stream &s) {
  ...
  case ZigValue::Kind::TaggedUnionType:
    s << "union(";
    if (ZigEnumType *tag_type = llvm::dyn_cast<ZigEnumType>(
...
void TypeSystemZig::DumpTypeDescription(opaque_compiler_type_t type, Stream &s,
                                        DescriptionLevel level) {
  ZigType *zig_type = UnwrapType(type);
  if (llvm::isa<ZigContainer>(zig_type->GetNamespace()))
...
llvm::APSInt TypeSystemZig::LoadTargetInt(ZigType *type,
  ...
  if (ZigIntType *int_type = llvm::dyn_cast<ZigIntType>(type))
    is_signed = int_type->IsSigned();
  llvm::APSInt value(bytes.size() * 8, !is_signed);
