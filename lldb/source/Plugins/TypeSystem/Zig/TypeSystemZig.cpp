...
#include "Plugins/ExpressionParser/Zig/ZigPersistentVariables.h"
...
static llvm::Align MaxIntAlignment(ArchSpec arch) {
  ...
  case llvm::Triple::ArchType::tcele64:
  ...
  case llvm::Triple::ArchType::riscv32be:
  ...
  case llvm::Triple::ArchType::riscv64be:
...
CompilerType TypeSystemZig::GetPointerDiffType(bool is_signed) {
  return GetSizeType(is_signed);
}
...
CompilerType TypeSystemZig::GetIntType(ConstString name, bool is_signed,
  ...
  bool is_named =
      llvm::StringSwitch<bool>(name)
          ...
          .Cases({"isize", "c_short", "c_int", "c_long", "c_longlong"},
                 is_signed)
...
bool TypeSystemZig::IsIndexableType(opaque_compiler_type_t type,
                                    bool *is_incomplete) {
  if (is_incomplete)
    *is_incomplete = false;
  if (!type)
...
bool TypeSystemZig::IsFloatingPointType(opaque_compiler_type_t type) {
  ZigType *zig_type = UnwrapType(type);
  return llvm::isa_and_present<ZigComptimeFloatType>(zig_type) ||
...
bool TypeSystemZig::IsMemberDataPointerType(opaque_compiler_type_t type) {
  UnwrapType(type);
  return false;
}
...
CompilerType TypeSystemZig::GetBasicTypeFromAST(BasicType basic_type) {
  ...
  case eBasicTypeFloat128: {
    ...
    case eBasicTypeLongDouble:
      ...
      break;
    case eBasicTypeFloat128:
      bit_size = target_info ? target_info->getFloat128Width() : 128;
      bit_align = target_info ? target_info->getFloat128Align() : 128;
      break;
    }
...
const llvm::fltSemantics &TypeSystemZig::GetFloatTypeSemantics(size_t byte_size,
                                                               Format format) {
...
Format TypeSystemZig::GetFormat(opaque_compiler_type_t type) {
  ...
  case ZigValue::Kind::FloatType:
    switch (llvm::cast<ZigFloatType>(zig_type)->GetBitSize()) {
    default:
      return eFormatFloat;
    case 128:
      return eFormatFloat128;
    }
  case ZigValue::Kind::OptionalType:
  case ZigValue::Kind::ErrorUnionType:
  case ZigValue::Kind::ArrayType:
...
size_t TypeSystemZig::GetIndexOfChildMemberWithName(
  ...
  if (ZigPointerType *zig_pointer_type =
    ...
    case ZigPointerType::Size::One:
      if (IsAggregateType(zig_pointer_type->GetChildType()->AsOpaqueType()))
        if (auto idx = llvm::expectedToOptional(GetIndexOfChildWithName(
  ...
  if (auto idx = llvm::expectedToOptional(
...
CompilerType
TypeSystemZig::GetDirectNestedTypeWithName(opaque_compiler_type_t type,
                                           llvm::StringRef name) {
  if (ZigType *zig_type = UnwrapType(type))
    if (ZigScope *zig_scope = zig_type->GetNamespace())
      if (std::vector<CompilerDecl> decls = DeclContextFindDeclByName(
...
ValueObjectSP
TypeSystemZig::CreateValueFromType(opaque_compiler_type_t type,
                                   ExecutionContextScope *exe_scope) {
...
bool TypeSystemZig::DumpValue(ZigValue *value, Stream &s, Format format) {
  if (!value)
    return false;
  if (format == eFormatInvalid)
    format = GetFormat(value->GetType()->AsOpaqueType());
  ...
  case ZigValue::Kind::ComptimeInt: {
    ...
    llvm::cast<ZigComptimeInt>(value)->GetValue().toString(
        str, radix, /*Signed=*/true, /*formatAsCLiteral=*/false, uppercase);
    s << str;
    return true;
  }
  case ZigValue::Kind::ComptimeFloat: {
    llvm::SmallString<256> str;
    llvm::APFloat float_value = llvm::cast<ZigComptimeFloat>(value)->GetValue();
    switch (format) {
    default:
      float_value.toString(str);
      break;
    case eFormatHexFloat:
      str.resize_for_overwrite(256);
      float_value.convertToHexString(str.data(), 0, false,
                                     llvm::APFloat::rmNearestTiesToEven);
      str.truncate(str.find(0));
      break;
    }
    s << str;
    return true;
...
bool TypeSystemZig::DumpTypeValue(opaque_compiler_type_t type, Stream &s,
  ...
  case eFormatFloat128:
    flt_semantics = &ZigFloatType::GetSemantics(128);
    break;
  case eFormatChar:
...
LLVM_DUMP_METHOD void TypeSystemZig::dump(opaque_compiler_type_t type) const {
  if (!type)
    return;
  llvm::errs() << UnwrapType(type)->GetQualifiedName() << '\n';
}
#endif
...
ZigScope *TypeSystemZig::UnwrapScope(void *opaque_decl_ctx) const {
  assert(VerifyDeclContext(opaque_decl_ctx) && "not owned by this");
  return static_cast<ZigScope *>(opaque_decl_ctx);
}
ZigScope *TypeSystemZig::UnwrapScope(CompilerDeclContext decl_ctx) const {
...
ZigDeclaration *TypeSystemZig::UnwrapDecl(void *opaque_decl) const {
  assert(VerifyDecl(opaque_decl) && "not owned by this");
  return static_cast<ZigDeclaration *>(opaque_decl);
}
ZigDeclaration *TypeSystemZig::UnwrapDecl(CompilerDecl decl) const {
...
ZigType *TypeSystemZig::UnwrapType(opaque_compiler_type_t type) const {
  assert(Verify(type) && "not owned by this");
  return static_cast<ZigType *>(type);
}
ZigType *TypeSystemZig::UnwrapType(CompilerType type) const {
  assert(type.GetTypeSystem().GetSharedPointer().get() == this &&
         "not owned by this");
...
ScratchTypeSystemZig::ScratchTypeSystemZig(Target &target)
    : TypeSystemZig("scratch ASTContext", target.GetArchitecture()),
      ...
      m_persistent_variables(new ZigPersistentVariables) {}
...
ScratchTypeSystemZig::GetPersistentExpressionState() {
