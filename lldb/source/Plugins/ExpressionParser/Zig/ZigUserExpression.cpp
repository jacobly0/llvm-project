...
#include "Plugins/Highlighter/Zig/ZigLexer.h"
...
ZigPersistentVariables *
GetPersistentExpressionState(ExecutionContext &exe_ctx) {
  if (Target *target = exe_ctx.GetTargetPtr())
    return llvm::cast_if_present<ZigPersistentVariables>(
        target->GetPersistentExpressionStateForLanguage(eLanguageTypeZig));
  return nullptr;
}
...
public:
  ...
  ValueObjectSP CompareExpr(ResultLoc &&rl = ResultLoc()) {
    ...
    case TK::RightArrowEqual: {
      ...
      case ZigValue::Kind::EnumLiteralType: {
        ValueObjectZig *lhs_zig = ValueObjectZig::dyn_cast(lhs.get());
        if (!lhs_zig)
          return nullptr;
        ZigEnumLiteral *lhs_enum_literal =
            llvm::dyn_cast_if_present<ZigEnumLiteral>(
                lhs_zig->UnwrapZigValue().second);
        if (!lhs_enum_literal)
          return nullptr;
        switch (rhs_zig_type->GetKind()) {
        default:
        ...
        case ZigValue::Kind::EnumType: {
          auto rhs_val_or_err = rhs->GetValueAsAPSInt();
          if (!rhs_val_or_err)
            return CreateValueObject(rhs_val_or_err.takeError());
          llvm::APSInt rhs_val =
              rhs_val_or_err->extOrTrunc(rhs_zig_type->GetBitSize());
          switch (op.kind) {
          default:
            llvm_unreachable("already checked");
          case TK::EqualEqual:
          ...
          case TK::RightArrowEqual:
            return CreateValueObject(Status::FromErrorStringWithFormatv(
                "operator {0} not allowed for type '{1}'", op.source,
                rhs_type.GetDisplayTypeName()));
          }
          for (const ZigTagField &field :
               llvm::cast<ZigTagType>(rhs_zig_type)->GetFields()) {
            if (field.GetName() != lhs_enum_literal->GetValue())
              continue;
            bool result;
            switch (op.kind) {
            default:
              llvm_unreachable("already checked");
            case TK::EqualEqual:
              result = field.GetValue()->GetValue() == rhs_val;
              break;
            case TK::ExclamationEqual:
              result = field.GetValue()->GetValue() != rhs_val;
              break;
            }
            return CreateValueObject(rhs_type_system->GetBool(result));
          }
          return CreateValueObject(Status::FromErrorStringWithFormatv(
              "no field named '{0}' in enum '{1}'",
              lhs_enum_literal->GetValue(), rhs_type.GetDisplayTypeName()));
        }
        }
        break;
      }
      case ZigValue::Kind::GeneratedTagType:
      case ZigValue::Kind::EnumType: {
        auto lhs_val_or_err = lhs->GetValueAsAPSInt();
        if (!lhs_val_or_err)
          return CreateValueObject(lhs_val_or_err.takeError());
        llvm::APSInt lhs_val =
            lhs_val_or_err->extOrTrunc(lhs_zig_type->GetBitSize());
        switch (rhs_zig_type->GetKind()) {
        default:
          break;
        case ZigValue::Kind::EnumLiteralType: {
          ValueObjectZig *rhs_zig = ValueObjectZig::dyn_cast(rhs.get());
          if (!rhs_zig)
            return nullptr;
          ZigEnumLiteral *rhs_enum_literal =
              llvm::dyn_cast_if_present<ZigEnumLiteral>(
                  rhs_zig->UnwrapZigValue().second);
          if (!rhs_enum_literal)
            return nullptr;
          switch (op.kind) {
          default:
          ...
          case TK::RightArrowEqual:
            return CreateValueObject(Status::FromErrorStringWithFormatv(
                "operator {0} not allowed for type '{1}'", op.source,
                lhs_type.GetDisplayTypeName()));
          }
          for (const ZigTagField &field :
               llvm::cast<ZigTagType>(lhs_zig_type)->GetFields()) {
            if (field.GetName() != rhs_enum_literal->GetValue())
              continue;
            bool result;
            switch (op.kind) {
            default:
              llvm_unreachable("already checked");
            case TK::EqualEqual:
              result = field.GetValue()->GetValue() == lhs_val;
              break;
            case TK::ExclamationEqual:
              result = field.GetValue()->GetValue() != lhs_val;
              break;
          ...
          return CreateValueObject(Status::FromErrorStringWithFormatv(
              "no field named '{0}' in enum '{1}'",
              rhs_enum_literal->GetValue(), lhs_type.GetDisplayTypeName()));
        }
        }
        break;
      }
        ...
        if (ConsumeToken(TK::DotDot)) {
          ...
          if (end_val) {
            ...
            if (ValueObjectZig *lhs_zig = ValueObjectZig::dyn_cast(lhs.get())) {
              ...
              Address ptr_addr(lhs->GetPointerValue().address);
              if ((!lhs_is_allowzero && !ptr_addr.GetOffset()) ||
                  !ptr_addr.Slide(offset))
                return nullptr;
              lhs = CreatePointerValueObject(
                  llvm::formatv("[{0:d}..{1:d}]", start_val, *end_val).str(),
                  pointer_type, ptr_addr);
            }
          } else {
            CompilerType pointer_type = type_system->GetPointerType(
            ...
            Address ptr_addr((llvm::isa<ZigPointerType>(lhs_zig_type)
                                  ? lhs->GetPointerValue()
                                  : lhs->GetAddressOf())
                                 .address);
            if ((!lhs_is_allowzero && !ptr_addr.GetOffset()) ||
              ...
              return nullptr;
            lhs = CreatePointerValueObject(
                llvm::formatv("[{0:d}..]", start_val).str(), pointer_type,
                ptr_addr);
          }
        } else {
  ...
  ValueObjectSP PrimaryTypeExpr(ResultLoc &&rl = ResultLoc()) {
    switch (m_token.kind) {
    case TK::Builtin:
      if (m_token.source == "@ptrCast" || m_token.source == "@alignCast" ||
          m_token.source == "@addrSpaceCast" ||
          m_token.source == "@constCast" || m_token.source == "@volatileCast") {
        ...
        Address ptr_addr(ptr_valobj->GetPointerValue().address);
        if (!ptr_addr.IsValid())
          return nullptr;
        return CreatePointerValueObject(ptr_valobj->GetName(), type, ptr_addr);
      }
      ...
      if (m_token.source == "@fromBackingInt" ||
          m_token.source == "@enumFromInt") {
        ResultLocType *rlt = llvm::dyn_cast<ResultLocType>(&rl);
        if (!rlt)
          return CreateValueObject(Status::FromErrorString(
              "@fromBackingInt must have a known result type"));
        auto type_system =
      ...
      if (m_token.source == "@backingInt" || m_token.source == "@intFromEnum") {
        if (!ConsumeNextToken(TK::LeftParen))
          return nullptr;
        ...
        return CreateValueObject(type_system->GetInt(
            int_type, enum_val->extOrTrunc(int_type->GetBitSize())));
      }
      if (m_token.source == "@intFromError") {
        ...
        return CreateValueObject(type_system->GetInt(
            int_type, error_val->extOrTrunc(int_type->GetBitSize())));
      }
      if (m_token.source == "@intFromPtr") {
        ...
        ZigIntType *int_type = llvm::cast<ZigIntType>(
            type_system->UnwrapType(type_system->GetSizeType(false)));
        return CreateValueObject(type_system->GetInt(
            int_type, ptr_val->extOrTrunc(int_type->GetBitSize())));
      }
    ...
    case TK::Dot:
      ...
      case TK::Identifier: {
        ...
        ZigEnumLiteral *enum_literal =
            type_system->GetEnumLiteral(ConstString(m_token.parsed));
        NextToken();
        return CreateValueObject(enum_literal);
      }
      default:
        break;
      }
    ...
    case TK::Identifier:
      ...
      return nullptr;
    case TK::NonZigIdentifier:
      if (PersistentExpressionState *persistent_state =
              GetPersistentExpressionState(m_exe_ctx)) {
        if (auto persistent_var =
                persistent_state->GetVariable(m_token.parsed)) {
          if (auto persistent_val = persistent_var->GetValueObject()) {
            NextToken();
...
private:
  ...
  ValueObjectSP CreatePointerValueObject(llvm::StringRef name,
                                         CompilerType type, Address addr) {
    return ValueObject::CreateValueObjectFromAddress(
        name, addr.GetLoadAddress(m_exe_ctx.GetTargetPtr()), m_exe_ctx, type,
        false);
  }
  ...
  static ValueObjectSP LookupLocalName(ConstString name, StackFrame *frame,
    ...
    for (CompilerDecl decl : found_decls) {
      ...
      for (VariableSP var : *vars)
        if (var->GetDecl() == decl)
          return var->CreateValueObject(frame);
    }
    return nullptr;
  }
  ...
  ValueObjectSP BinaryOp(ValueObjectSP lhs, Token op, ValueObjectSP rhs) {
    ...
    case ZigValue::Kind::PointerType:
      ...
      case ZigValue::Kind::IntType: {
        ...
        case ZigPointerType::Size::Many:
        case ZigPointerType::Size::C: {
          Address lhs_ptr_addr(lhs->GetPointerValue().address);
          if (!lhs_ptr_addr.IsValid())
            return nullptr;
          bool success;
          ...
          default:
            break;
          case TK::Plus:
            if (lhs_ptr_addr.Slide(offset))
              return CreatePointerValueObject("", res_type, lhs_ptr_addr);
            return nullptr;
          case TK::Minus:
            if (lhs_ptr_addr.Slide(-offset))
              return CreatePointerValueObject("", res_type, lhs_ptr_addr);
...
ExpressionResults ZigUserExpression::DoExecute(
  ...
  if (PersistentExpressionState *persistent_state =
          GetPersistentExpressionState(exe_ctx))
    if (auto persistent_var =
            persistent_state->GetVariable(m_result->GetName());
        persistent_var && persistent_var->GetValueObject() == m_result)
      result = persistent_var;
    else
      result =
          persistent_state->CreatePersistentVariable(std::move(m_result)->Clone(
