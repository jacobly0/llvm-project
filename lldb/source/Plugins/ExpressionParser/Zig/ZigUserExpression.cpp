...
public:
  ZigParser(ExecutionContext &exe_ctx, const char *source)
      : m_exe_ctx(exe_ctx), m_lexer(source), m_skip(false) {
    NextToken();
  }
    ...
    while (true)
      ...
      case TK::LeftBracket: {
        ...
        if (ZigPointerType *lhs_pointer_type =
          ...
          elem_type = lhs_pointer_type->GetChildType();
          switch (lhs_pointer_type->GetSize()) {
          case ZigPointerType::Size::One:
            if (ZigSequenceType *lhs_sequence_type =
                    llvm::dyn_cast<ZigSequenceType>(elem_type)) {
              elem_type = lhs_sequence_type->GetChildType();
        ...
        CompilerType usize_type = type_system->GetSizeType(false);
        NextToken();
        ValueObjectSP start = Expr(ResultLocType(usize_type));
        ...
        if (ConsumeToken(TK::DotDot)) {
          ...
          if (m_token.kind == TK::RightBracket) {
            if (lhs_len && start_val > *lhs_len + (lhs_sentinel != nullptr))
              end_val = *lhs_len;
          } else {
            ValueObjectSP end = Expr(ResultLocType(usize_type));
          ...
          uint64_t offset = elem_type->GetByteSize() * start_val;
          if (end_val) {
            CompilerType array_type = type_system->GetArrayType(
                *end_val - start_val,
                *end_val == lhs_len ? lhs_sentinel : nullptr,
                elem_type->AsOpaqueType());
            ...
            CompilerType pointer_type = type_system->GetPointerType(
                ZigPointerType::Size::One, nullptr, lhs_is_allowzero,
                commonAlignment(lhs_pointer_align, offset), lhs_addrspace,
                lhs_is_const, lhs_is_volatile, elem_type->AsOpaqueType());
            addr_t ptr_addr;
      ...
      case TK::Dot:
        ...
        case TK::QuotedIdentifier:
          lhs = lhs->GetChildMemberWithName(m_token.parsed);
          if (!lhs || lhs->GetError().Fail())
            return lhs;
          NextToken();
          break;
    ...
    case TK::Builtin:
      ...
      if (m_token.source == "@ptrCast") {
        ...
        if (!ptr_type || ptr_type->GetSize() == ZigPointerType::Size::Slice)
          return nullptr;
        ValueObjectSP ptr = Expr(ResultLocType(type));
        if (!ptr || ptr->GetError().Fail())
          return ptr;
        if (ValueObjectZig *ptr_zig = ValueObjectZig::dyn_cast(ptr.get())) {
          auto ptr_zig_value = ptr_zig->UnwrapZigValue().second;
          if (!ptr_zig_value)
            return nullptr;
          ZigPointer *zig_ptr = llvm::dyn_cast<ZigPointer>(ptr_zig_value);
          if (!zig_ptr)
            return nullptr;
          ptr = CreateValueObject(type_system->GetPointer(
              ptr_type, zig_ptr->GetPointee(), zig_ptr->GetOffset()));
        } else {
          addr_t ptr_addr = ptr->GetPointerValue();
          if (ptr_addr == LLDB_INVALID_ADDRESS)
            return nullptr;
          ptr = ValueObject::CreateValueObjectFromAddress(
              ptr->GetName(), ptr_addr, m_exe_ctx, type, false);
        }
        if (!ptr || ptr->GetError().Fail())
          return ptr;
        if (!ConsumeToken(TK::RightParen))
    ...
    case TK::NumberLiteral: {
      ...
      if (str.getAsInteger(radix, int_val)) {
        ...
        if (auto convert_result = float_val.convertFromString(
                m_token.parsed, llvm::APFloat::rmNearestTiesToEven);
            !convert_result)
...
private:
  ...
  Token NextToken() { return m_token = m_lexer.NextToken(); }
  ...
  ValueObjectSP BinaryOp(ValueObjectSP lhs, Token op, ValueObjectSP rhs) {
    ...
    case ZigValue::Kind::ComptimeIntType: {
      ...
      case ZigValue::Kind::ComptimeIntType: {
        ...
        case TK::PlusPercent:
        case TK::PlusPipe: {
          unsigned bits = 1 + std::max(lhs_val->getSignificantBits(),
                                       rhs_val->getSignificantBits());
        ...
        case TK::AsteriskPipe: {
          unsigned bits =
              lhs_val->getSignificantBits() + rhs_val->getSignificantBits();
          return CreateValueObject(lhs_type_system->GetComptimeInt(
              lhs_val->extOrTrunc(bits) * rhs_val->extOrTrunc(bits)));
        }
        case TK::Slash: {
          ...
          unsigned bits = std::max(1 + lhs_val->getSignificantBits(),
                                   rhs_val->getSignificantBits());
          return CreateValueObject(lhs_type_system->GetComptimeInt(
              lhs_val->extOrTrunc(bits) / rhs_val->extOrTrunc(bits)));
        }
        case TK::Percent: {
          ...
          unsigned bits = std::max(lhs_val->getSignificantBits(),
                                   rhs_val->getSignificantBits());
          return CreateValueObject(lhs_type_system->GetComptimeInt(
              lhs_val->extOrTrunc(bits) % rhs_val->extOrTrunc(bits)));
        }
...
bool ZigUserExpression::Parse(DiagnosticManager &diagnostic_manager,
  ...
  return bool(m_result = ZigParser(exe_ctx, Text()).RootExpr());
}
